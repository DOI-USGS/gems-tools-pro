...
Edited slightly for use with GeMS FGDC tools
changing paths to locations of modules and commenting out imports and functions that
are not needed
...
# for use with gems, xml_utils, utils are in different location,
# don't need data_io
#from pymdwizard.core.xml_utils import xml_node
from Resources.xml_utils import xml_node
...
def get_bounding(fname, feature_class=None):
    """
    ...
    """
    # for some reason, calling get_layer causes my jupyter notebook to crash, even when the
    # function has been moved to the notebook itself. But if the contents of get_layer are
    # included in this function, it works
    #layer = get_layer(fname, feature_class)
    ...
    if fname.endswith(".shp"):
        ...
        driver = ogr.GetDriverByName("GPKG")
        db = driver.Open(fname, 0)
        layer = db.GetLayerByName(feature_class)
    else:
        # it better be a raster
        layer = gdal.Open(fname)    
